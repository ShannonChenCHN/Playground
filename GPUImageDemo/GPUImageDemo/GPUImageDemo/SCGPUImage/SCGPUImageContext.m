...
extern dispatch_queue_attr_t SCGPUImageDefaultQueueAttribute(void);  // MARK: 这也可以？

@interface SCGPUImageContext () {
...
@implementation SCGPUImageContext
...
- (id)init;
{
    ...
    _contextQueue = dispatch_queue_create("com.sunsetlakesoftware.GPUImage.openGLESContextQueue", SCGPUImageDefaultQueueAttribute());
...
+ (SCGPUImageContext *)sharedImageProcessingContext;
{
    ...
    static SCGPUImageContext *sharedImageProcessingContext = nil;
...
{
    [[SCGPUImageContext sharedImageProcessingContext] useAsCurrentContext];
}
...
{
    SCGPUImageContext *sharedContext = [SCGPUImageContext sharedImageProcessingContext];
    [sharedContext setContextShaderProgram:shaderProgram];
}

- (void)setContextShaderProgram:(SCGLProgram *)shaderProgram;
{
    EAGLContext *imageProcessingContext = [self context];
    if ([EAGLContext currentContext] != imageProcessingContext)
    {
...
+ (BOOL)deviceSupportsRedTextures;
{
    ...
    dispatch_once(&pred, ^{
        supportsRedTextures = [SCGPUImageContext deviceSupportsOpenGLESExtension:@"GL_EXT_texture_rg"];
    });
...
- (SCGLProgram *)programForVertexShaderString:(NSString *)vertexShaderString fragmentShaderString:(NSString *)fragmentShaderString {
    ...
    SCGLProgram *programFromCache = [shaderProgramCache objectForKey:lookupKeyForShaderProgram];
    
    if (programFromCache == nil)
    {
        programFromCache = [[SCGLProgram alloc] initWithVertexShaderString:vertexShaderString fragmentShaderString:fragmentShaderString];
...
- (SCGPUImageFramebufferCache *)framebufferCache;
{
    if (_framebufferCache == nil)
    {
        _framebufferCache = [[SCGPUImageFramebufferCache alloc] init];
    }
